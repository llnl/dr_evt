...
PYBIND11_MODULE(dr_evt, m) {
    ...
    py::enum_<RunTimeMode>(
        m, "RunTimeMode",
        "Controls how a job's actual run time is selected; the scheduler uses its time limit for planning.")
    ...
    py::enum_<PriorityPolicy>(m, "PriorityPolicy", "Selects the waiting-job priority policy.")
    ...
    py::class_<Sim_Params>(m, "SimParams")
        ...
        .def_readwrite("infile", &Sim_Params::m_infile,
                       "str: Path to the input trace used by batch mode and initialize_trace().")
    ...
    py::class_<Simulation::Statistics>(m, "Statistics")
        ...
        .def_readonly("jobs_completed", &Simulation::Statistics::jobs_completed, "int: Number of completed jobs.")
        .def_readonly("jobs_running", &Simulation::Statistics::jobs_running, "int: Number of running jobs.")
        .def_readonly("jobs_waiting", &Simulation::Statistics::jobs_waiting, "int: Number of waiting jobs.")
        .def_readonly("current_time", &Simulation::Statistics::current_time, "float: Current simulation time.")
        .def_readonly("total_nodes", &Simulation::Statistics::total_nodes, "int: Configured node capacity.")
        .def_readonly("nodes_in_use", &Simulation::Statistics::nodes_in_use, "int: Nodes allocated to running jobs.")
        .def_readonly("nodes_available", &Simulation::Statistics::nodes_available, "int: Currently unallocated nodes.")
        .def_readonly("utilization", &Simulation::Statistics::utilization, "float: Node utilization in the range [0, 1].")
        .def_readonly("avg_wait_time", &Simulation::Statistics::avg_wait_time, "float: Mean completed-job wait time.")
        .def_readonly("avg_turnaround_time", &Simulation::Statistics::avg_turnaround_time, "float: Mean completed-job turnaround time.")
        .def_readonly("makespan", &Simulation::Statistics::makespan, "float: Time from first submission to final completion.")
    ...
    // FCFS/EASY reservation snapshot for evaluating in-process backfill candidates.
    ...
    py::class_<Simulation::Backfill_Window>(m, "BackfillWindow")
        ...
        .def_readonly("releases", &Simulation::Backfill_Window::releases,
                      "list[ResourceRelease]: Future resource-release events in time order.");
    ...
    py::class_<Simulation>(m, "Simulation")
        ...
        .def("append_job", &Simulation::append_job,
             ...
             "Args:\n"
             "    submit_time (float): Arrival time, not earlier than current time.\n"
        ...
        .def("get_fcfs_head_shadow_time", &Simulation::get_fcfs_head_shadow_time,
             "Return the FCFS-head predicted start time as float, or -1.0 if no job waits.")

        .def("get_backfill_window", &Simulation::get_backfill_window,
             "Return a BackfillWindow snapshot for evaluating a backfill candidate.")
