 ...
 */
template <size_t N> class Multi_Platform_Runs {
public:
...
public:
   ...
   * @return true if the file was opened and parsed; false for an empty or
   * unreadable filename.
...
template <size_t N> void Multi_Platform_Runs<N>::reserve(const size_t n) {
  m_data.reserve(n);
}
...
bool Multi_Platform_Runs<N>::load(const std::string &ifname) {
  ...
  std::getline(ifs, line); // Consume the header line
  {
    ...
    if (hdr_strs.size() != N + 1) {
      std::string err = "Number of columns with line " + std::to_string(cnt) +
...
template <size_t N>
const typename Multi_Platform_Runs<N>::data_t &
Multi_Platform_Runs<N>::get_data() const {
  return m_data;
}
