...
ScheduleWindows::ScheduleWindows(num_nodes_t total_nodes,
    ...
    : m_total_nodes(total_nodes), m_job_data_ptr(&job_data) {}
...
void ScheduleWindows::add_reservation(job_no_t job_idx, sim_time_t start_time,
...
void ScheduleWindows::trim(sim_time_t current_time) {
  ...
  m_windows.erase(std::remove_if(m_windows.begin(), m_windows.end(),
                                 [current_time](const Window &g) {
                                   return g.end < current_time;
                                 }),
                  m_windows.end());
...
void ScheduleWindows::update_windows(job_no_t job_idx, sim_time_t start_time,
                                     sim_time_t end_time, num_nodes_t nodes,
                                     int op) {
...
void ScheduleWindows::consolidate_windows() {
  ...
  for (size_t i = 1; i < m_windows.size(); ++i) {
    ...
    if (current.start <= last.end &&
        current.available_nodes == last.available_nodes) {
...
std::vector<Window>
ScheduleWindows::get_ending_windows(sim_time_t start_time) const {
