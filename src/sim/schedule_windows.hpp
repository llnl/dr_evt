...
/**
 * @brief Represents a scheduling window (available time/resource slot) in the
 * schedule
 *
 * A window is a time period during which a certain number of nodes are
 * available for scheduling jobs. Used for finding backfill opportunities.
 */
struct Window {
 ...
 * where resources are available, enabling efficient backfill opportunity
 * detection.
 *
 * A scheduling window represents a time period with available resources.
 * As jobs are added/removed, windows are updated to reflect resource
 * availability.
 */
...
public:
   ...
   */
  ScheduleWindows(num_nodes_t total_nodes, const Trace::trace_data_t &job_data);
   ...
   */
  std::vector<Window> get_windows(sim_time_t start_time, tdiff_t length,
