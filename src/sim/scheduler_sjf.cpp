...
SJFScheduler::SJFScheduler(num_nodes_t total_nodes, const Trace &job_data,
                           BackfillPolicy backfill_policy)
    : SchedulerBase(total_nodes, job_data, backfill_policy),
      m_current_tracked_time(0.0) {}

void SJFScheduler::insert_job(job_no_t job_id, sim_time_t submit_time,
...
void SJFScheduler::update_eligible_jobs(sim_time_t current_time) {
  ...
  for (auto &pair : m_wait_queue) {
    ...
    if (entry.submit_time <= current_time &&
        entry.submit_time > m_current_tracked_time) {
...
std::multimap<tdiff_t, SJFScheduler::JobEntry>::iterator
SJFScheduler::find_fcfs_head() {
  ...
  for (auto it = m_wait_queue.begin(); it != m_wait_queue.end(); ++it) {
    ...
    if (m_eligible_jobs.count(entry.job_id) == 0)
      continue;
...
std::vector<job_no_t>
SJFScheduler::schedule(num_nodes_t free_nodes,
  ...
  if (head_nodes <= free_nodes) {
    ...
    // multimap::erase(iterator) is O(1) amortized regardless of the
    // node's position in the tree, unlike erasing from the middle of
    // a sequential container, so there's no performance reason to
    // defer this the way FCFSScheduler's deque-based lazy deletion
    // was worth doing.
  ...
  // Scan in SJF order (shortest first) for backfill candidates
  for (auto it = m_wait_queue.begin(); it != m_wait_queue.end(); ++it) {
    if (it == fcfs_head_it)
      continue; // Skip FCFS head
