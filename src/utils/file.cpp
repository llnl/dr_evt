...
/** @brief Return whether a filesystem entry exists at the supplied path. */
 ...
 *    File name is the same as the model name except that the extension is
 * `.out` and the file is going be under current working directory.
 */
...
std::string get_subpath(const std::string &basepath,
  ...
  if (((dir_match.length() == basepath.length()) && (basepath.back() == '/')) ||
      ((dir_match.length() == basepath.length() + 1) &&
       (dir_match.back() == '/'))) {
    return stem + ext;
  }
...
/** @brief Flush the directory containing a path after creating an entry. */
bool sync_directory(
    const std::string
        &path) { // Flush new directory entry https://lwn.net/Articles/457671/
  ...
  if ((odir_fd = open(odir, O_RDONLY)) < 0) {
    ...
    if (fsync(odir_fd) < 0) {
      std::cerr << "Cannot flush the directory " + std::string(odir)
                << std::endl;
      rc = false;
    }
    if (close(odir_fd) < 0) {
      std::cerr << "Cannot close the directory " + std::string(odir)
                << std::endl;
      rc = false;
    }
  }
...
/** @brief Flush a C++ output stream through to its underlying file descriptor.
 */
