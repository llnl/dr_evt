...
/** @brief Enable GNU CPU-affinity declarations required by this file. */
#define __USE_GNU 1
#endif
...
/** @brief Get the number of online processing units.
 * @return Number of online cores or hardware threads, or a negative value if
 * the operating-system query fails. */
...
/** @brief Read the calling thread's CPU affinity mask.
 * @param[out] cpus Processing-unit identifiers present in the affinity mask.
 * @return Zero on success, or the negative status from `sched_getaffinity()`.
 */
...
std::string to_string_omp_schedule_kind(int kind) {
  ...
  case static_cast<unsigned>(0x1):
    return "omp_sched_static";
  case static_cast<unsigned>(0x2):
    return "omp_sched_dynamic";
  case static_cast<unsigned>(0x3):
    return "omp_sched_guided";
  case static_cast<unsigned>(0x4):
    return "omp_sched_auto";
  case static_cast<unsigned>(0x80000000u):
    return "omp_sched_auto";
  }
#endif // defined(_OPENMP)
  return "unknown";
...
void set_static_schedule() {
  ...
  if (kind != omp_sched_static) {
    ...
    msg =
        "change " + to_string_omp_schedule_kind(kind) + " to omp_sched_static";
  } else {
