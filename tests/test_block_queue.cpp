...
template <size_t BlockSize> void run_tests_impl() {
  ...
  // Test 1: Insert jobs
  queue.insert_job(1, 0.0, 100.0,
                   10); // job_id=1, submit=0, run_time=100, nodes=10
  queue.insert_job(2, 5.0, 50.0,
                   20); // job_id=2, submit=5, run_time=50, nodes=20
  queue.insert_job(3, 10.0, 150.0,
                   15); // job_id=3, submit=10, run_time=150, nodes=15
  ...
  std::cout << "✓ Found and removed backfill candidate: job " << *candidate
            << std::endl;
  // Note: candidate was automatically removed
  assert(queue.active_count() == 2); // One job removed
  std::cout << "✓ Combined find-and-remove worked, active count: "
            << queue.active_count() << std::endl;
  ...
  assert(candidate.has_value());
  assert(*candidate == 1);
  std::cout << "✓ Resource constraint filtering works: job " << *candidate
            << std::endl;
  ...
  assert(candidate.has_value());
  assert(*candidate == 2);
  std::cout << "✓ Time constraint filtering works: job " << *candidate
            << std::endl;
  ...
  std::cout << "✓ Manual remove works, active count: " << queue4.active_count()
            << std::endl;
  ...
  std::cout << std::endl;
  assert(count ==
         2); // 2 jobs still active (one was removed by find_and_remove)
  ...
  std::cout << "Stats - blocks skipped (time): " << stats.blocks_skipped_time
            << std::endl;
  std::cout << "Stats - blocks skipped (resource): "
            << stats.blocks_skipped_resource << std::endl;
  ...
  std::cout << "✓ Multiple blocks created for " << (BlockSize + 1) << " jobs"
            << std::endl;
  ...
  assert(candidate.has_value());
  std::cout << "✓ Search across multiple blocks works: job " << *candidate
            << std::endl;
